...
public class DrawPig {
    ...
    private static final Color OUTLINE     = new Color(40, 90, 30);
    ...
    /** Draws the green minion pig into an ARGB image (transparent bg) and returns it. */
    public BufferedImage render() {
        ...
        // ---- ears (behind head so they peek out), head ----
        g.drawImage(circleLayer(44, 24, 10, body, true), 0, 0, null);
        g.drawImage(circleLayer(76, 24, 10, body, true), 0, 0, null);
        g.drawImage(circleLayer(60, 66, 44, body, true), 0, 0, null);

        // ---- eyes ----
        g.drawImage(circleLayer(46, 54, 11, white, true), 0, 0, null);
        g.drawImage(circleLayer(74, 54, 11, white, true), 0, 0, null);
        g.drawImage(circleLayer(48, 56, 4, dark, false), 0, 0, null);
        g.drawImage(circleLayer(72, 56, 4, dark, false), 0, 0, null);

        // ---- eyebrows + mouth (thin dark-green strokes) ----
        { BufferedImage s = strokeLayer(); Graphics gs = s.getGraphics();
          useColor(gs, dgreen);
          bresenhamLine(gs, 36, 40, 54, 44);                  // brows
          bresenhamLine(gs, 84, 40, 66, 44);
          bresenhamLine(gs, 50, 92, 70, 92);                  // mouth
          gs.dispose(); g.drawImage(s, 0, 0, null); }

        // ---- snout + nostrils ----
        g.drawImage(ellipseLayer(60, 74, 18, 14, snout, true), 0, 0, null);
        g.drawImage(ellipseLayer(53, 74, 2, 4, dark, false), 0, 0, null);
        g.drawImage(ellipseLayer(67, 74, 2, 4, dark, false), 0, 0, null);
    ...
    public static void main(String[] args) {
        DrawPig d = new DrawPig(120, 120);
        final BufferedImage img = d.render();
        JFrame f = new JFrame("Pig");
        f.add(new JPanel() {
